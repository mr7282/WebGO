...
func main() {
	...
	urls := flag.String("urls", strings.Join(defaultLinks, comma), "list of urls where each url separated via comma")
	query := flag.String("query", "a", "substring for searching")
	flag.Parse()

	result, err := SiteSearch(*query, strings.Split(*urls, comma))
	if err != nil {
