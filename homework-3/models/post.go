...
var PostWhere = struct {
	...
	Name:      whereHelperstring{field: "`post`.`name`"},
	Post:      whereHelperstring{field: "`post`.`post`"},
	...
	DeletedAt: whereHelpertime_Time{field: "`post`.`deleted_at`"},
}
...
var (
	postAllColumns            = []string{"ID", "name", "post", "user_id", "created_at", "updated_at", "deleted_at"}
	postColumnsWithoutDefault = []string{"name", "post", "user_id", "deleted_at"}
...
func (postL) LoadUser(ctx context.Context, e boil.ContextExecutor, singular bool, maybePost interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		foreign.R.UserPosts = append(foreign.R.UserPosts, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.UserID == foreign.ID {
				...
				foreign.R.UserPosts = append(foreign.R.UserPosts, local)
				break
			}
		}
	}
...
func (o *Post) SetUser(ctx context.Context, exec boil.ContextExecutor, insert bool, related *User) error {
	...
	if related.R == nil {
		related.R = &userR{
			UserPosts: PostSlice{o},
		}
	} else {
		related.R.UserPosts = append(related.R.UserPosts, o)
	}
